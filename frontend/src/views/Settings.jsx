...
import { emailLogin, emailRegister, IS_ANDROID } from '../lib/api.js'
...
export default function Settings() {
  ...
  const signInHere = () => useUI.getState().openSheet(close => <EmailAuthInline close={close} mode="signin" setUser={setUser} pushState={pushState} pullState={pullState} toast={toast} />)
  const registerHere = () => useUI.getState().openSheet(close => <EmailAuthInline close={close} mode="signup" setUser={setUser} pushState={pushState} pullState={pullState} toast={toast} />)
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host openGym')} subtitle={t('Email sign-in, sync across your devices, your own data.')} accessory="chevron"
        ...
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Signed in — data syncs to this account.')} />
...
// The same email/password sign-in and account-creation flow as the login screen's, reached
// from Settings instead (used from guest mode, without leaving the app).
function EmailAuthInline({ close, mode, setUser, pushState, pullState, toast }) {
  ...
  const go = async () => {
    ...
    try {
      ...
      setUser(u); close()
      if (mode === 'signup' && hasData(useStore.getState().S)) { await pushState(); toast(t('Account created — data moved into it')) }
      else { await pullState(); toast(mode === 'signup' ? t('Welcome, {0}', u.name) : t('Welcome back, {0}', u.name)) }
    } catch (e) { toast(e.message || t('Something went wrong')) }
  }
  return <>
    <h3>{mode === 'signup' ? t('Create your account') : t('Sign in')}</h3>
    ...
    <TextField type="password" autoComplete={mode === 'signup' ? 'new-password' : 'current-password'}
      placeholder={t('Password')} value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && go()} />
    <div style={{ height: 12 }} /><Button variant="primary" onClick={go}>{mode === 'signup' ? t('Create account') : t('Sign in')}</Button>
  </>
}
